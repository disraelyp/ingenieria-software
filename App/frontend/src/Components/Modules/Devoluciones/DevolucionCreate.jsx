...
import devolucionServices from '../../../Services/devolucion'
  ...
  const crearDevolucion = async (event) => {
    ...
    if(cliente !== null) {
      if(rows.length !== 0) {
        ...
        const devolucion = await devolucionServices.create(nuevaDevolucion)
        if(devolucion.data.error){
          console.log(devolucion.data.message)
        } else {
          dispatch(initializeDevoluciones())
          dispatch(createNotification('Devolucion creada correctamente!', 'Información'))
          history.push('/Facturacion/Devoluciones')
        }
      } else {
        dispatch(createNotification('Necesita ingresar almenos un producto.', 'Error'))
      }
    } else {
      dispatch(createNotification('Necesita ingresar un cliente, para crear una devolucion.', 'Error'))
    }
  }
...
const DevolucionCreate = () => {
  ...
  useEffect(() => {
    dispatch(updateTitulo('Crear Devolucion'))
  }, [dispatch])
