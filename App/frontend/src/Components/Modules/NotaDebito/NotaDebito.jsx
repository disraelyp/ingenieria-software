...
const NotaDebito = () => {
  ...
  const notaDebito = notasDebito.find(nota => parseInt(nota.ID) === parseInt(id))
  const proveedor = notaDebito.Proveedor
  const Fecha = useDatePicker('date', notaDebito.FechaCreacion, () => {return null})
  ...
  useEffect(() => {
    dispatch(updateTitulo('Ver nota de debito'))
  }, [dispatch])
  ...
  if(!notaDebito){
    ...
    if(rows.length === 0) {
      notaDebito.Productos.map(producto => agregarFila(producto.CodigoBarra, producto.Descripcion, producto.Cantidad, producto.Costo, producto.Cantidad*producto.Costo))
    }
  }
  return (
    <>
