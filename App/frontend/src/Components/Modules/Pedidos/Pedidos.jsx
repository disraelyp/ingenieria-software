  ...
  return (
    <>
      ...
      <Grid container spacing={2}>
        <Grid item xs={6}>
          <Button onClick={() => history.push(soloAdministradorAlmacenista(user.user.Role) ? '/Facturacion/Pedidos/Crear' : '/Facturacion')} disable={!soloAdministradorAlmacenista(user.user.Role)} text={'Crear'} color='success' />
          <Button onClick={() => history.push(soloAdministradorAlmacenista(user.user.Role) ? '/Facturacion/Pedidos/Editar/'+selectionModel[0] : '/Facturacion')} text={'Modificar'} disable={selectionModel.length === 0 ? true : !soloAdministrador(user.user.Role)} color='primary'  />
        </Grid>
        <Grid item xs={6}>
          ...
          <Button onClick={() => history.push(soloAdministradorAlmacenista(user.user.Role) ? '/Facturacion/Pedidos/Ver/'+selectionModel[0] : '/Facturacion')} disable={selectionModel.length === 0} text={'Ver'} color='secondary' />
        </Grid>
      </Grid>
    </>
  )
